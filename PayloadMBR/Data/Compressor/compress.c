...
int main(int argc, char **argv) {
	...
	if (!infile) {
		fprintf(stderr, "Could open input file for reading.\n");
		return 2;
	}
	...
	if (fsize >= BUFFER_SIZE) {
		fprintf(stderr, "The input file is good for compressing!\n");
		return 3;
	} else if (fsize <= 0) {
		fprintf(stderr, "The input file is full of junk.\n");
		return 3;
	}
	...
	if (!outfile) {
		fprintf(stderr, "Could open output file for writing.\n");
		return 2;
	}
